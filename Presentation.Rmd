...
```{r}
mySummary5 %>% 
  ggplot(aes(fct_reorder(domain,surprise %>% desc),surprise))+
  geom_bar(stat='identity')+
  xlab("Language Code")
```
...
```{r}
mySummary5 %>% 
  ggplot(aes(fct_reorder(domain,anger %>% desc),anger))+
  geom_bar(stat='identity')+
  xlab("Language Code")
```
...
```{r}
mySummary5 %>% 
  ggplot(aes(fct_reorder(domain,disgust %>% desc),disgust))+
  geom_bar(stat='identity')+
  xlab("Language Code")
```
...
- Referendum was deemed illegal by the Spanish courts, so *illegal* might be such a word.
